...
def plot_metrics(episodes_rewards, losses, paths):
    ...
    plt.plot(pd.Series(episodes_rewards).rolling(100).mean(), label='Rewards MA@50', color='green', linewidth=3)
    plt.title('Rewards')
    plt.legend()
    plt.grid(True)
    plt.savefig(paths[0])
    ...
    plt.plot(pd.Series(losses).rolling(100).mean(), label='Losses MA@50', color='orange', linewidth=3)
