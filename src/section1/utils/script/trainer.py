...
def episode_step(env, ac_agent, max_steps: int = 1000):
    ...
    if done:
        loguru.logger.success(
            f"Episode ended successfully after {step_count} steps with final reward: {sum(episode_reward):.2f}"
        )
    elif truncated:
        loguru.logger.warning(
            f"Episode truncated after {step_count} steps with final reward: {sum(episode_reward):.2f}"
        )
    elif step_count >= max_steps:
        loguru.logger.error(
            f"Episode terminated due to max steps ({max_steps}) with final reward: {sum(episode_reward):.2f}"
        )
